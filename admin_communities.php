...
<th>Sajt/forum</th>
<th width="150">Vlasnik</th>
<th width='100'>Akcije</th>

<?php

    $kveri = mysql_query("SELECT * FROM community");
...
$i++;
 ...
 if($sql_num > 1){ echo "<tr> <td>$i.</td> <td><a href='/community_info/$srv[id]'>$name</a></td> <td>$sql_num</td> <td>$broj_igraca/$max_igraca</td> <td><a target='_blank' href='http://$srv[forum]'>$srv[forum]</a></td> <td>$srv[owner]</td> <td><a href='/admin_process.php?task=delete_comm&id=$srv[id]' title='Obrisi server'><img src='/img/btn_del.png'></a> <a title='Restartuj rank' href='/admin_process.php?task=reset_comm&id=$srv[id]'><img src='/img/reset_img.png'></a>  <!-- <a title='Edituj server' href='/admin/community_edit/$srv[id]'><img src='/img/Edit.png'></a> --> </td></tr>"; } else { $i = $i-1; }
}
?>
</table>
