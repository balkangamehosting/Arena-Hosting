...
$port		= $ip_port[1];
        ...
        $image = imagecreatefrompng("img/gt/baner_arena_crveni.png");
		imagecopy($image, $chart_img, 425, 35, 0, 0, 130, 50);
		imagecopy($image, $server_location, 529, 113, 0, 0, 22, 15);
       
        $white = imagecolorallocate($image, 255, 255, 255);
		imagettftext($image, 7, 0, 133, 122, $white, 'fonts/arial.ttf', "$game");
        imagettftext($image, 7, 0, 253, 47, $white, 'fonts/arial.ttf', "$name");
		imagettftext($image, 7, 0, 254, 86, $white, 'fonts/arial.ttf', "$ip:$port");
		imagettftext($image, 7, 0, 253, 126, $white, 'fonts/arial.ttf', "$mapa");
        imagettftext($image, 7, 0, 320, 126, $white, 'fonts/arial.ttf', "$info[num_players]/$info[max_players]");
		imagettftext($image, 7, 0, 387, 126, $white, 'fonts/arial.ttf', "#$info[rank]");
		imagettftext($image, 7, 0, 455, 126, $white, 'fonts/arial.ttf', "$status");
